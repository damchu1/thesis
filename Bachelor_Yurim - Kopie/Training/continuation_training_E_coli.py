...
from Transformer.transformer_wip import Transformer
from Transformer.auxiliary_file import train_validate_test_split
from Transformer.tokenisation import format_dataset, \
    N_AA_TOKENS, N_CODON_TOKENS


# The constant `MAX_SEQ_LEN` must be adjusted to the data set currently
# dealt with!
# In the case of the E. coli data set comprising 6 genomes, the maximum
# sequence length was set to 500
...
# sign must not be used within Python scripts in order to denote a
# variable
...
# This is due to the fact that merely a proof of concept is supposed to
# be achieved
num_layers = 3
d_model = 128
dff = 512
...
# The Adam optimiser is employed in conjunction with a custom learning
# rate scheduler
# The learning rate scheduler follows the formula in the original
# Transformer paper
...
# Load the weights from epoch 43, the last epoch from which the weights
# were completely saved before the interruption occurred
transformer.load_weights(
    "Checkpoint_directory_intermediate_weights/best_weights.weights.h5"
)
...
training_callbacks = [
    ...
    # Add a checkpoint callback in order to save the model weights
    # periodically
...
# After completion of the training, the weights are pickled and saved to
# disk
# Apart from that, the specifications of the transformer architecture
# are saved as well
...
with open("E-coli-6-genomes-trained-weights_6th_approach_window_width_51.pkl", "wb") as f:
    pickle.dump(
